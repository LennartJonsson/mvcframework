<?php
/**
 * A model for managing MvcFrameWork modules.
 *
...
class CMModules extends CObject
{
   ...
   * Properties
   */
  private $MvcFrameWorkCoreModules = array('CMvcFrameWork', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $MvcFrameWorkCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CTextFilter');
  ...
  public function ReadAndAnalyse()
  {
	...
	if(class_exists($module))
	{
	  ...
	  $modules[$module]['isMvcFrameWorkCore']   = in_array($rc->name, array('CMvcFrameWork', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
	  $modules[$module]['isMvcFrameWorkCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
	}
      }
    }
    $dir->close();
  ...
  private function GetDetailsOfModule($module)
  {
    ...
    if(class_exists($module))
    {
      ...
      $details['isMvcFrameWorkCore'] = in_array($rc->name, $this->MvcFrameWorkCoreModules);
      $details['isMvcFrameWorkCMF']  = in_array($rc->name, $this->MvcFrameWorkCMFModules);
